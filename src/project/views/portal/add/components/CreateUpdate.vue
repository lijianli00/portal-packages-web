<template>
  <div>
    <sh-form :model="form" ref="form" labelWidth="120px" className="xy-form">
      <sh-form-item label="所属应用" prop="appId" rules="required">
        <el-select v-model="form.appId" placeholder="请选择">
          <el-option v-for="item in getData" :key="item.appId" :label="item.appName" :value="item.appId"> </el-option>
        </el-select>
      </sh-form-item>
...
export default {
  ...
  created() {
    this.getData = []
    this.$post(portal.list, { appId: '', appName: '', limit: 99, page: 1 }, res => {
